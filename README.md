...
- **Home screen widget** — an input-field-style row plus mic and send buttons. Tapping
  opens a lightweight quick-compose overlay with the keyboard already up (Android
  `RemoteViews` cannot host real text input, so the overlay is the typing surface). The
  mic button starts system speech recognition.
...
- **Per-memo visibility override** — tap the visibility chip in the compose overlay to
  cycle PRIVATE → PROTECTED → PUBLIC.

Text memos only in v0.1 — image/video attachments are a planned follow-up (Memos 0.31
...
- Networking: OkHttp + kotlinx.serialization against `/api/v1`
  (`POST /memos`, `GET /instance/profile`, `GET /auth/me`), Bearer PAT auth.
- Settings: DataStore preferences, auto-saved.
